...
public class Philosopher extends Thread {
	...
	public void run() {
		...
		// generate random sleep time with milliseconds
		Random random = new Random();
		int randTime = random.nextInt(10000);
		
		// philosophers arrive at the table at random
		try {
		...
		// philosophers gather around the table
		// barrier is implemented here
		try {
			for(int i = 0; i < N; i++) {
				
				// up processes not equal to the id
				// block all processes except the id
				if(id != i) {
					barriers[i].release();
				}
			}
			// down on the selected id
			for(int i = 0; i < (N-1); i++) {
				// let the other processes catch up
				barriers[id].acquire();
			}
		}
		catch(Exception e) {
		...
		// dinner starts with selected id
		table.StartDining_GUI(id);
		 ...
		 while(true) { 
			 ...
			 table.StopEating_GUI(id); // dining ends on selected id
		 }
	}	
	
	public void take_forks(int id) throws Exception {
		// enter critical region
		mutex.acquire();
		state[id] = HUNGRY;
		table.Hungry_GUI(id);
		test(id); // try to acquire to forks
		mutex.release(); // exit critical region
		// block with semaphores
		sem[id].acquire();
	}
	
	public void put_forks(int id) throws Exception {
		// enter critical region
		mutex.acquire();
		state[id] = THINKING;
		table.ForkPut_GUI(id);
		test((id + N - 1) % N); // test left
		test((id+1) % N); // test right
		mutex.release(); // exit critical region
	}
	
	public void think(int id) throws Exception {
		// generate random thought time in milliseconds - [0, 10000] 
		Random random = new Random();
		int randTime = random.nextInt(10000);
		Thread.sleep(randTime);
	}
	...
	public void test(int id) throws Exception {
		if(state[id] == HUNGRY && state[(id + N - 1) % N] != EATING && state[(id + 1) % N] != EATING) {
			...
			// up on semaphore for locking others
			sem[id].release();
		}
	}
